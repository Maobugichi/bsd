...
import { motion, AnimatePresence } from "motion/react"; // Add AnimatePresence here (or from 'framer-motion' if that's your lib)
    ...
    <>
      ...
      <AnimatePresence>
        {show && (
          ...
          >
              <div className="h-20 flex justify-between items-center px-6 border-b border-solid border-gray-200/50 bg-white/80 backdrop-blur-lg sticky top-0 z-10"> {/* Slightly taller for better touch, sticky for scroll */}
                  <Logo />
                  <button 
                      onClick={closeNav}
                      className="p-3 hover:bg-gray-100/50 rounded-full transition-colors"
                  >
                      <X size={32} />
                  </button>
              </div>
              ...
              <ul className="flex flex-col gap-2 p-6 mt-8">
                  ...
                  <li key={item.key}>
                      ...
                      className={`
                          ...
                          ${activeTab === item.key
                          ? 'bg-gradient-to-r from-black to-gray-900 text-white shadow-lg shadow-black/20' 
                          : 'text-gray-700 hover:bg-gray-100/60 hover:shadow-sm'
                          }
                      `}
                      >
                      {item.name}
                  ...
                  <button 
                      className="w-full px-6 py-4 rounded-xl bg-gradient-to-r from-black to-gray-900 text-white text-base font-medium hover:shadow-lg hover:shadow-black/20 transition-all duration-300"
                  >
                      Contact Us
                  </button>
                  </li>
